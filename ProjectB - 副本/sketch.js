...
function draw() {
  ...
  //sun
  if (isStarted && currentMode === 'christmas') {
    let drumVolume = christmasShakingSound.getVolume();
    let circleSize = map(drumVolume, 0, 2, 1200, 2000);
    
    if (faces.length > 0) {
        let face = faces[0];
        let noseX = width - face.keypoints[4].x;
        let noseY = face.keypoints[4].y - 200;
        
        let hue = map(noseX, 0, width, 20, 40);
        
        for (let i = 3; i >= 0; i--) {
            drawingContext.shadowBlur = 50 + i * 20;
            drawingContext.shadowColor = `rgba(255, ${150 + i * 20}, 0, ${0.2 - i * 0.03})`;
            
            push();
            noStroke();
            let gradient = drawingContext.createRadialGradient(
                noseX, noseY, 0,
                noseX, noseY, circleSize/2
            );
            gradient.addColorStop(0, `hsla(${hue}, 100%, ${60 + i * 10}%, ${0.4 - i * 0.05})`);
            gradient.addColorStop(1, `hsla(${hue}, 100%, ${60 + i * 10}%, 0)`);
            
            drawingContext.fillStyle = gradient;
            circle(noseX, noseY, circleSize * (1 + i * 0.2));
            pop();
        }
        ...
        noStroke();
        fill(hue, 100, 70, 0.6);
        circle(noseX, noseY, circleSize * 0.7);
        pop();
  ...
  if (isStarted && currentMode === 'NowWe') {
    // Remove the wave lines code entirely
  } else if (isStarted && currentMode === 'christmas') {
    let drumVolume = christmasShakingSound.getVolume();
    let circleSize = map(drumVolume, 0, 2, 1200, 2000);
