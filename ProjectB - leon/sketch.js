...
let currentHeadX = 0;  // To store head position
...
class WaveTrack {
  ...
  draw(strokeW) {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      let pointyWave = pow(sin(i * 0.3), 9) * amplitude * 1.5;
      curveVertex(x, this.y + pointyWave);
    }
    endShape();
    pop();
  }
...
function draw() {
  ...
  // Add offset from the black background edge
  let arrowDrawX = arrow.x;
  if (arrow.x > width/2) {
    arrowDrawX = arrow.x - 30; // Move arrow 30px left when on right side
  } else {
    arrowDrawX = arrow.x + 30; // Move arrow 30px right when on left side
  }

  if (isDragging) {
    arrow.x = constrain(mouseX, 50, width - 50);
  }
  if (isStarted && mouseX >= width - 50) {
    isStarted = false;
  ...
  for (let i = 0; i < faces.length; i++) {
    ...
    currentHeadX = noseX;  // Store the head position
  ...
  if (!isDragging) {
    arrowOffset = sin(frameCount * 0.05) * 10;
  } else {
    arrowOffset = 0;
  }

  if (arrow.x > width / 2) {
    let x1 = (arrowDrawX + arrowOffset) + cos(angleRad / 2) * lineLength;
    let y1 = arrow.y - sin(angleRad / 2) * lineLength;
    let x2 = (arrowDrawX + arrowOffset) + cos(-angleRad / 2) * lineLength;
    let y2 = arrow.y - sin(-angleRad / 2) * lineLength;
    line(arrowDrawX + arrowOffset, arrow.y, x1, y1);
    line(arrowDrawX + arrowOffset, arrow.y, x2, y2);

    let x3 = (arrowDrawX + arrowOffset - spacing) + cos(angleRad / 2) * lineLength;
    let y3 = arrow.y - sin(angleRad / 2) * lineLength;
    let x4 = (arrowDrawX + arrowOffset - spacing) + cos(-angleRad / 2) * lineLength;
    let y4 = arrow.y - sin(-angleRad / 2) * lineLength;
    line(arrowDrawX + arrowOffset - spacing, arrow.y, x3, y3);
    line(arrowDrawX + arrowOffset - spacing, arrow.y, x4, y4);
  } else {
    let x1 = (arrowDrawX + arrowOffset) - cos(angleRad / 2) * lineLength;
    let y1 = arrow.y - sin(angleRad / 2) * lineLength;
    let x2 = (arrowDrawX + arrowOffset) - cos(-angleRad / 2) * lineLength;
  ...
  if (isStarted && currentMode === 'NowWe') {
    let strokeW = map(currentHeadX, 0, width, 8, 1); 
...
function mousePressed() {
  // Increase the hit area size
  let hitAreaWidth = arrow.size * 5;  // Changed from 2.5 to 5
  let hitAreaHeight = arrow.size * 4; // Changed from 2 to 4

  if (arrow.x > width / 2) {
    if (mouseX > arrow.x - hitAreaWidth/2 && mouseX < arrow.x + hitAreaWidth/2 &&  // Centered the hit area
